...
# def add_path(path: str | None, root_call: Call):
...
#                 call_entry = {'name': func_name, 'count': execution_counters[key]}
...
#                         f_path = filter_path(path, ignored, renamed)
#                         add_path(f_path, signal.trace[0])
#                     call_stack.pop()
...
# ignored = ["<listcomp>", '<lambda>', "__init__"]
# ignored += ["gate", "outgoing", 'step', '<genexpr>', "bitlist", "_bitlist_from_value", "test"]
...
#     return a+b+c, res3
# # inp, out = test()
# inp, out = trace(test, ignored, skipped, renamed)
# graph = compiled_from_io(inp, out)
# print_traces(graph)
