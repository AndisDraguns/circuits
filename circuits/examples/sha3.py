...
def copy_lanes(
    lanes: list[list[list[Bit]]], lanes_tmp: list[list[list[Bit]]]
) -> list[list[list[Bit]]]:
    for x in range(5):
        for y in range(5):
...
def lanes_to_state(lanes: list[list[list[Bit]]]) -> list[Bit]:
    ...
    for x in range(5):
        for y in range(5):
            state[64 * (x + 5 * y) : 64 * (x + 5 * y) + 64] = reverse_bytes(lanes[x][y])
    return state
...
def state_to_lanes(state: list[Bit]) -> list[list[list[Bit]]]:
    ...
    for x in range(5):
        for y in range(5):
            lanes[x][y] = reverse_bytes(state[64 * (x + 5 * y) : 64 * (x + 5 * y) + 64])
    return lanes
